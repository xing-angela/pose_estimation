...
def render_image(vertices, faces, images, cameras, colors=None, add_back=True):
    
    render_data = {}
    assert vertices.shape[1] == 3 and len(vertices.shape) == 2, 'shape {} != (N, 3)'.format(vertices.shape)
    pid = 0
    render_data[pid] = {'vertices': vertices, 'faces': faces, 'colors': [c/255.0 for c in [106, 90, 205]] if colors is None else colors/255,
        'vid': pid, 'name': 'object'}
